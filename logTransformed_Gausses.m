...
for j = 1:57
    ...
    for i = 1:3065
        if(ytrain(i,1) == 1)
            count_Y1 = count_Y1+1;
            Y1_XTrain(count_Y1,j) = XlogTrain(i,j);
        else 
            count_Y0 = count_Y0+1;
            Y0_XTrain(count_Y0,j) = XlogTrain(i,j);
        end
    end     
end
P_Y1 = count_Y1/3065;
...
for j = 1:57
    for i = 1:3065
        PXj_Y1_train(i,j) = normpdf(XlogTrain(i,j),mu_Y1_XTrain(j),sigma_Y1_XTrain(j));
        PXj_Y0_train(i,j) = normpdf(XlogTrain(i,j),mu_Y0_XTrain(j),sigma_Y0_XTrain(j));
    end
end
%apply normpdf function to normalize Xtest data.
for j = 1:57
    for i = 1:1536
        PXj_Y1_test(i,j) = normpdf(XlogTest(i,j),mu_Y1_XTrain(j),sigma_Y1_XTrain(j));
        PXj_Y0_test(i,j) = normpdf(XlogTest(i,j),mu_Y0_XTrain(j),sigma_Y0_XTrain(j));
    end
end
