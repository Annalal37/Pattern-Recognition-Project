...
for lambda = [1:1:10,15:5:100]  %change lambda according to the CA1 requirement.
    ...
    mu = 1./(1+exp(-Wk'*znorm_Xtrain'));
    S = diag(mu.*(1-mu));
    g_reg = znorm_Xtrain'*(mu' - ytrain) + lambda*Wk;  %apply lambda to iterate 
    H_reg = znorm_Xtrain'*S*znorm_Xtrain + lambda* I;
    a = Wk; %store the previous Wk
    Wk = a - inv(H_reg)*g_reg;    %iterate

    while(max(abs(W0_first-b)>0.00001))    %set a theshold 0.01 to detect the last W0 in the iteration     
    ...
    while(max(abs(Wk-a)>0.00001))  %set a theshold 0.01 to detect the last Wk in the iteration 
    mu = 1./(1+exp(-Wk'*znorm_Xtrain'));   %iterate using lambda
    S = diag(mu.*(1-mu));
    g_reg = znorm_Xtrain'*(mu' - ytrain) + lambda*Wk;
    H_reg = znorm_Xtrain'*S*znorm_Xtrain + lambda* I;
    a = Wk;
    Wk = a - inv(H_reg)*g_reg;
    end
    %calculate train error rate 
    result_train = W0_first*Xtrain_first'+Wk'* znorm_Xtrain';  %calculate log(P(Y=1|Xj)/P(Y=0|Xj))
    ...
    result_test = W0_first*Xtest_first'+Wk'* znorm_Xtest';
...
axis([1 100 0.08 0.11])
title('Relationship between error rate and lambda through z-normalized data');
xlabel('lambda');
ylabel('error rate');
