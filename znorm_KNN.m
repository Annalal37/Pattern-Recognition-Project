...
for i = 1:3065
    distance_test(i,h) = 0;
    for j = 1:57
    distance_test(i,h) = distance_test(i,h)+(znorm_Xtest(h,j)-znorm_Xtrain(i,j)).^2;
    end
end
end
...
%count the Euclidean distances mapping from each feature in each row of Xtrain data to each feature in each row of Xtrain data 
 for h = 1:3065
    for i = 1:3065
        ...
        for j = 1:57
        distance_train(i,h) = distance_train(i,h)+(znorm_Xtrain(h,j)-znorm_Xtrain(i,j)).^2;
        end
    end
 end 
...
axis([1 100 0 0.16]);
title('Relationship between error rate and k through z-normalized strategy');
xlabel('k');
ylabel('error rate');
