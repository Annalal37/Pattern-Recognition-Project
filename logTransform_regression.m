...
%insert one volume to the original data
...
for lambda = [1:1:10,15:5:100]  %change lambda according to the CA1 requirement.
    ...
    mu = 1./(1+exp(-Wk'*XlogTrain'));
    S = diag(mu.*(1-mu));
    g_reg = XlogTrain'*(mu' - ytrain) + lambda*Wk;  %apply lambda to iterate 
    H_reg = XlogTrain'*S*XlogTrain + lambda* I;
    ...
    mu = 1./(1+exp(-Wk'*XlogTrain'));   %iterate using lambda
    S = diag(mu.*(1-mu));
    g_reg = XlogTrain'*(mu' - ytrain) + lambda*Wk;
    H_reg = XlogTrain'*S*XlogTrain + lambda* I;
    a = Wk;
    Wk = a - inv(H_reg)*g_reg;
    end
    %calculate train error rate 
    result_train = W0_first*Xtrain_first'+Wk'* XlogTrain';  %calculate log(P(Y=1|Xj)/P(Y=0|Xj))
    ...
    result_test = W0_first*Xtest_first'+Wk'* XlogTest';
...
axis([1 100 0.04 0.1])
title('Relationship between error rate and lambda through log-transformed data');
xlabel('lambda');
ylabel('error rate');
